...
Use `--task-file` for a newline-separated selection; omitting task selectors
requests the full pinned dataset. Choose the full-run timeout and concurrency
...
reasons. `run_manifest.json` records the seed, benchmark, image labels, selected
IDs and applied configuration; `result_index.jsonl` retains every selected task,
...
shared daemon. After an interrupted process, inspect any campaign `active_run`
and its resources before resuming; the launcher refuses unresolved ownership.

For infrastructure recovery, add `--resume-from OLD_RUN_ROOT` to a new run with
the same seed and configuration. Settled results, including genuine failures,
are skipped. Preserve old directories and combine compatible result records only
