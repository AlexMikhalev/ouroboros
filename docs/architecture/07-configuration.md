...
| OUROBOROS_MODEL_CONTEXT_WINDOWS | "{}" | Role-owned context sizing assertions; zero means Auto, not a provider limit, and no value here is reviewer authority |
...
| OUROBOROS_CONTEXT_MODE | max | Context mode (`nano`/`low`/`max`), owner-selected outside Cyber Pro; `nano` is the compact owner projection and is recorded as `owner_nano`/`rendered_mode=nano` in physical usage facts. It sizes Ouroboros's OWN working window and never whether its changes are reviewed: scope review applies in every mode (BIBLE P1/P3). Cyber may configure it through the same audited writer. |
...
GigaChat provider specifics (`gigachat::`): routed through the native `gigachat` library, not OpenAI-compatible (`llm.py::_chat_gigachat`). OpenAI `tools` map to GigaChat `functions`; at most ONE `function_call` returns per turn, so parallel `tool_calls` collapse to the first; role `tool` results become role `function` and must be valid JSON (plain text wrapped as `{"result": ...}`); the `system` message must be first, so later system-reminders demote to `user`. `reasoning_effort` is deliberately omitted — hidden reasoning can consume the whole output budget and return empty content. GigaChat exposes no automatic live cost source, so cost stays nullable/unknown rather than a hand-maintained tariff. A small window is no longer a scope-review barrier: scope review retrieves, so a GigaChat scope row runs the inspection episode on its own window — slowly, because GigaChat returns at most one function call per turn — and its read coverage is diagnostic, never an authority floor (BIBLE P3). Incomplete or unobserved reads preserve the responding reviewer and its findings; the agent judges whether to read more. A host that cannot project the inspection tools at all still returns a typed route failure (`native_inspection_unavailable`), not a completed review. The blocking triad still reviews the full staged diff either way.
